...
# ============================================================
# MnraPanel Universal Installer / Manager
# ============================================================

APP_DIR="${MNRA_APP_DIR:-/opt/MnraPanel}"
REPO="${MNRA_REPO:-https://github.com/amirh00sain/MnraPanel.git}"
BRANCH="${MNRA_BRANCH:-main}"

INSTALLER_URL="${MNRA_INSTALLER_URL:-https://raw.githubusercontent.com/amirh00sain/MnraPanel/main/start.sh}"

ENV_FILE="/etc/mnra-panel.env"
SERVICE="mnra-panel"

PORT="8080"

VENV="$APP_DIR/.venv"
PIDFILE="$APP_DIR/mnrapanel.pid"
LOGFILE="$APP_DIR/mnrapanel.log"

CLI="/usr/local/bin/mnrapanel"
...
UV_VERSION="${MNRA_UV_VERSION:-0.12.9}"

# Xray version.
XRAY_VERSION="${MNRA_XRAY_VERSION:-26.3.27}"
...
root() {
    ...
    f="$(mktemp /tmp/mnrapanel-root.XXXXXX)"
...
download_repo() {
    ...
    TMP_ROOT="$(mktemp -d /tmp/mnrapanel.XXXXXX)"

    log "Downloading MnraPanel..."
...
install_uv() {
    ...
    tmp="$(mktemp -d /tmp/mnrapanel-uv.XXXXXX)"
...
start_panel() {
    ...
    if running; then
        ...
        ok "MnraPanel is running."

        return 0
    fi


    printf '[ERROR] MnraPanel failed to start.\n' >&2
...
info_panel() {
    ...
    echo "URL: http://127.0.0.1:8080/mnra"


    if [[ -n "$local_ip" ]]; then

        echo "Local URL: http://${local_ip}:8080/mnra"
    ...
    if [[ -n "$ip" ]]; then
        ...
        echo "Public URL: http://${ip}:8080/mnra"
    ...
    then
        ...
        echo "Codespace URL: https://${CODESPACE_NAME}-8080.${domain}/mnra"
    ...
    echo "  mnrapanel info"
    echo "  mnrapanel status"
    echo "  mnrapanel start"
    echo "  mnrapanel stop"
    echo "  mnrapanel restart"
    echo "  mnrapanel update"
    echo "  mnrapanel logs"
    echo "  mnrapanel uninstall"
...
update_panel() {
    ...
    f="$(mktemp /tmp/mnrapanel-update.XXXXXX)"
...
uninstall_panel() {
    ...
    echo "This will remove MnraPanel."
...
if [[ ! -f "$APP/start.sh" ]]; then
    echo "MnraPanel is not installed."
    exit 1
fi
...
EOF
    ...
    ok "Global command installed: mnrapanel"
}
...
main() {
    ...
    case "${1:-install}" in
        ...
        *)
            ...
            echo
            echo "MnraPanel Universal Installer"
            echo
            echo "Usage:"
            echo
