// Mnra Panel — VLESS Worker (ESM Module)
// ══════════════════════════════════════════════════════════════════════════════
...
async function getUser(env, uuid) {
  ...
  try {
    const raw = await env.MNRA_KV.get('user:' + uuid);
    if (!raw) return null;
...
async function setUser(env, uuid, u) {
  await env.MNRA_KV.put('user:' + uuid, JSON.stringify(u));
}
...
function tunnelEnv(env) {
  return { MNRA_KV: env.TUNNEL_KV || env.MNRA_KV };
}
...
function reverseEnv(env) {
  return { MNRA_KV: env.REVERSE_KV || env.MNRA_KV };
}
...
async function getIpList(env, uuid) {
  try {
    const raw = await env.MNRA_KV.get('ips:' + uuid);
    return raw ? JSON.parse(raw) : null;
  } catch (e) { return null; }
}
async function setIpList(env, uuid, rec) {
  try { await env.MNRA_KV.put('ips:' + uuid, JSON.stringify(rec)); } catch (e) {}
}
...
async function getCountryLocation(env, code) {
  try {
    const raw = await env.MNRA_KV.get('proxies') || '[]';
...
export default {
  async fetch(request, env, ctx) {
    ...
    // ── Health / Debug ──
    if (path === '/health' || path === '/') {
      return new Response('Mnra VLESS Worker online', { headers: { 'content-type': 'text/plain' } });
    }
...
// ── Admin API (Bearer PANEL_TOKEN) ──
    ...
    if (path === '/panel/config' && request.method === 'POST') {
      ...
      const existing = await env.MNRA_KV.list({ prefix: 'user:' });
      const keep = new Set();
      for (const u of users) {
        ...
        keep.add('user:' + uuid);
        await env.MNRA_KV.put('user:' + uuid, JSON.stringify(rec));
        written++;
      ...
      for (const k of existing.keys) {
        if (!keep.has(k.name)) await env.MNRA_KV.delete(k.name);
      }
      if (body.routes && typeof body.routes === 'object') {
        const locations = Array.isArray(body.routes.locations) ? body.routes.locations : [];
        await env.MNRA_KV.put('proxies', JSON.stringify(locations));
      }
      if (body.settings && typeof body.settings === 'object') {
        await env.MNRA_KV.put('settings', JSON.stringify(body.settings));
      }
      await env.MNRA_KV.put('heartbeat', JSON.stringify({ at: Date.now(), users: written }));
    ...
    if (path === '/panel/status' && request.method === 'GET') {
      ...
      const list = await env.MNRA_KV.list({ prefix: 'user:' });
      const now = Date.now() / 1000;
      for (const k of list.keys) {
        try {
          const u = JSON.parse(await env.MNRA_KV.get(k.name));
          if (!u) continue;
          users++;
    ...
    if (path.startsWith('/api/')) {
      ...
      if (path === '/api/users' && request.method === 'GET') {
        ...
        const list = await env.MNRA_KV.list({ prefix: 'user:' });
        for (const k of list.keys) {
          const raw = await env.MNRA_KV.get(k.name);
      ...
      if (path.startsWith('/api/user/')) {
        ...
        if (request.method === 'DELETE') {
          await env.MNRA_KV.delete('user:' + uuid);
          return json({ ok: true });
        }
        const u = await getUser(env, uuid);
      ...
      if (path === '/api/proxies' && request.method === 'POST') {
        const body = await request.json();
        await env.MNRA_KV.put('proxies', JSON.stringify(body.locations || []));
        return json({ ok: true });
      }
